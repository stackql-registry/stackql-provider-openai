...
**1. Split** - `npm run split --provider-name openai`. Candidate services (inventory decides, recorded in `service_names.json`): `models`, `files`, `fine_tuning` (jobs, events, checkpoints, checkpoint permissions), `batches`, `vector_stores` (stores, files, file batches), `assistants` (deprecation-labelled family), `evals`, `uploads` (metadata), `containers` (if clean).
